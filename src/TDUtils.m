...
NSArray *appList(void) {
  ...
  [installedApplications
      enumerateObjectsUsingBlock:^(LSApplicationProxy *proxy, NSUInteger idx,
                                   BOOL *stop) {
        ...
        if (!bundleID || !name || !version || !executable)
          return;
  ...
  for (int i = 0; i < numberOfProcesses; ++i) {
    ...
    if (strlen(pathBuffer) > 0) {
      ...
      NSString *processName =
          [[NSString stringWithUTF8String:pathBuffer] lastPathComponent];
      NSDictionary *dict = [[NSDictionary alloc]
          initWithObjects:[NSArray arrayWithObjects:processID, processName, nil]
                  forKeys:[NSArray arrayWithObjects:@"pid", @"proc_name", nil]];
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [[UIApplication sharedApplication]
            launchApplicationWithIdentifier:bundleID
                                  suspended:YES];
        sleep(1);
        ...
        if (pid == -1) {
          dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSLog(@"[trolldecrypt] failed to get pid for binary name: %@",
                  binaryName);

            errorController = [UIAlertController
                alertControllerWithTitle:@"Error: -1"
            ...
            UIAlertAction *okAction = [UIAlertAction
                actionWithTitle:NSLocalizedString(@"Ok", @"Ok")
                        ...
                        handler:^(UIAlertAction *action) {
                          ...
                          [errorController dismissViewControllerAnimated:NO
                                                              completion:nil];
            ...
            [root presentViewController:errorController
                               animated:YES
                             completion:nil];
          });
...
void bfinject_rocknroll(pid_t pid, NSString *appName, NSString *version) {
  NSLog(
      @"[trolldecrypt] Spawning thread to do decryption in the background...");
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        DumpDecrypted *dd = [[DumpDecrypted alloc]
            initWithPathToBinary:[NSString stringWithUTF8String:fullPathStr]
                         appName:appName
                      appVersion:version];
        if (!dd) {
        ...
        NSLog(@"[trolldecrypt] Full path to app: %s   ///   IPA File: %@",
              fullPathStr, [dd IPAPath]);
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
          ...
          alertController = [UIAlertController
              alertControllerWithTitle:@"Decrypting"
                               message:@"Please wait, this will take a few "
                                       @"seconds..."
                        preferredStyle:UIAlertControllerStyleAlert];
          ...
          if ([kw respondsToSelector:@selector(topmostPresentedViewController)])
            root =
                [kw performSelector:@selector(topmostPresentedViewController)];
          else
            root = [kw rootViewController];
          root.modalPresentationStyle = UIModalPresentationFullScreen;
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
          ...
          if ([[UIApplication sharedApplication]
                  canOpenURL:[NSURL URLWithString:@"filza://"]]) {
            UIAlertAction *openAction = [UIAlertAction
                actionWithTitle:@"Show in Filza"
                          style:UIAlertActionStyleDefault
                        handler:^(UIAlertAction *action) {
                          [kw removeFromSuperview];
                          kw.hidden = YES;
                          ...
                          NSString *urlString = [NSString
                              stringWithFormat:@"filza://view%@", [dd IPAPath]];
                          [[UIApplication sharedApplication]
          ...
          [root presentViewController:doneController
                             animated:YES
                           completion:nil];
        }); // dispatch on main
...
NSArray *decryptedFileList(void) {
  ...
  while (file = [directoryEnumerator nextObject]) {
    if ([[file pathExtension] isEqualToString:@"ipa"]) {
      ...
      NSDictionary *fileInfo =
          @{@"fileName" : file, @"modificationDate" : modificationDate};
  ...
  NSArray *sortedFiles =
      [files sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
...
NSString *docPath(void) {
  ...
  [[NSFileManager defaultManager]
            createDirectoryAtPath:@"/var/mobile/Library/TrollDecrypt/decrypted"
...
void decryptAppWithPID(pid_t pid) {
  ...
  if (!bundleID) {
    error = @"Error: -2";
    message =
        [NSString stringWithFormat:@"Failed to get bundle id for pid: %d", pid];
  }
  ...
  if (!app) {
    error = @"Error: -3";
    message = [NSString
        stringWithFormat:@"Failed to get LSApplicationProxy for bundle id: %@",
                         bundleID];
  }
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    UIAlertController *alert = [UIAlertController
        alertControllerWithTitle:@"Decrypt"
                         message:[NSString stringWithFormat:@"Decrypt %@?",
                                                            appInfo[@"name"]]
                  preferredStyle:UIAlertControllerStyleAlert];
...
void github_fetchLatedVersion(
    NSString *repo, void (^completionHandler)(NSString *latestVersion)) {
  NSString *urlString = [NSString
      ...
      completionHandler:^(NSData *data, NSURLResponse *response,
        ...
        if (!error) {
          if ([response isKindOfClass:[NSHTTPURLResponse class]]) {
            ...
            if (!jsonError) {
              NSString *version =
                  [json[@"tag_name"] stringByReplacingOccurrencesOfString:@"v"
                                                               withString:@""];
...
void fetchLatestTrollDecryptVersion(
    void (^completionHandler)(NSString *version)) {
