/*
    bfinject - Inject shared libraries into running App Store apps on iOS 11.x
   < 11.2 https://github.com/BishopFox/bfinject

    Carl Livitt @ Bishop Fox

        Based on code originally by 10n1c:
   https://github.com/stefanesser/dumpdecrypted/blob/master/dumpdecrypted.c Now
   with the following enhancements:
        - Dump ALL encrypted images in the target application: the app itself,
   its frameworks, etc.
        - Create a valid .ipa containing the decrypted binaries. Save it in
   ~/Documents/decrypted-app.ipa
        - The .ipa can be modified and re-signed with a developer cert for
   redeployment to non-jailbroken devices
...
uint8_t *readProcessMemory(mach_port_t proc, mach_vm_address_t addr,
...
static kern_return_t readmem(mach_vm_offset_t *buffer,
                             mach_vm_address_t address, mach_vm_size_t size,
                             vm_map_t targetTask,
  ...
  kr = mach_vm_region(port, &address_info, &size_info, VM_REGION_BASIC_INFO_64,
  ...
  kr = mach_vm_read_overwrite(port, address, size, (mach_vm_address_t)buffer,
                              &nread);
  if (kr) {
    ...
    fprintf(stderr,
            "[ERROR] vm_read failed! requested size: 0x%llx read: 0x%llx\n",
            size, nread);
    return KERN_FAILURE;
  }
  return KERN_SUCCESS;
...
int64_t get_image_size(mach_vm_address_t address, vm_map_t targetTask,
                       uint64_t *vmaddr_slide) {
  vm_region_basic_info_data_64_t region_info = {0};
  // allocate a buffer to read the header info
  // NOTE: this is not exactly correct since the 64bit version has an extra 4
  // bytes but this will work for this purpose so no need for more complexity!
  struct mach_header header = {0};
  if (readmem((mach_vm_offset_t *)&header, address, sizeof(struct mach_header),
  ...
  if (readmem((mach_vm_offset_t *)loadcmds, address + mach_header_size,
              header.sizeofcmds, targetTask, &region_info)) {
...
int find_off_cryptid(const char *filePath) {
  ...
  struct load_command *lc =
      (struct load_command *)((mach_vm_address_t)header +
...
- (id)initWithPathToBinary:(NSString *)pathToBinary
  ...
  // [self setDocPath:[NSString stringWithFormat:@"%@/Documents",
  // NSHomeDirectory()]];
...
- (void)makeDirectories:(const char *)encryptedImageFilenameStr {
  ...
  // Find start of image path, relative to the base of the app sandbox (ie.
  // /var/mobile/.../FooBar.app/THIS_PART_HERE)
  ...
  NSString *path = [NSString
      stringWithFormat:@"%s/ipa/Payload/%s", docPath, lastPartOfAppPath];
  ...
  if (![fm createDirectoryAtPath:path
          withIntermediateDirectories:true
                           attributes:nil
                                error:&err]) {
    NSLog(@"[trolldecrypt] WARNING: make_directories failed to make directory "
          @"%@. Error: %@",
          path, err);
  }
  ...
  snprintf(self->decryptedAppPathStr, PATH_MAX, "%s/%s", [path UTF8String],
           self -> filename);
...
- (BOOL)dumpDecryptedImage:(vm_address_t)imageAddress
  ...
  struct mach_header header = {0};
  vm_region_basic_info_data_64_t region_info = {0};
  if (readmem((mach_vm_offset_t *)&header, imageAddress,
              sizeof(struct mach_header), targetTask, &region_info)) {
  ...
  if (header.magic == MH_MAGIC_64) {
    // lc = (struct load_command *)((unsigned char *)header + sizeof(struct
    // mach_header_64));
    readmem((mach_vm_offset_t *)lc,
            imageAddress + sizeof(struct mach_header_64), header.sizeofcmds,
            targetTask, &region_info);
    NSLog(@"[trolldecrypt] detected 64bit ARM binary in memory.\n");
  } else if (header.magic ==
             MH_MAGIC) { /* we might want to check for other errors here, too */
    // lc = (struct load_command *)((unsigned char *)header + sizeof(struct
    // mach_header));
    readmem((mach_vm_offset_t *)lc, imageAddress + sizeof(struct mach_header),
            header.sizeofcmds, targetTask, &region_info);
  ...
  for (i = 0; i < image_mh->ncmds; i++) {
    if (lc->cmd == LC_ENCRYPTION_INFO || lc->cmd == LC_ENCRYPTION_INFO_64) {
      ...
      // struct encryption_info_command *eic = (uint8_t*)malloc(sizeof(struct
      // encryption_info_command);); readmem((mach_vm_offset_t*)eic,
      // imageAddress+sizeof(struct mach_header_64), header.sizeofcmds,
      // targetTask, &region_info);

      const char *appFilename = strrchr(encryptedImageFilenameStr, '/');
      if (appFilename == NULL) {
        NSLog(@"[trolldecrypt] There are no / in the filename. This is an "
              @"error.\n");
        return false;
      }
      appFilename++;
      ...
      NSLog(@"[trolldecrypt] offset to cryptid (%d) found in memory @ %p (from "
            @"%p). off_cryptid = %u (0x%x)\n",
            eic->cryptid, &eic->cryptid, image_mh, off_cryptid, off_cryptid);
      // NSLog(@"[trolldecrypt] Found encrypted data at offset %u 0x%08x.
      // image_mh @ %p. cryptedData @ 0x%x. cryptsize = %u (0x%x) bytes.\n",
      // eic->cryptoff, eic->cryptoff, image_mh, (unsigned int)image_mh +
      // eic->cryptoff, eic->cryptsize, eic->cryptsize);
      ...
      if (n != sizeof(buffer)) {
        NSLog(@"[trolldecrypt] Warning read only %d of %lu bytes from "
              @"encrypted file.\n",
              n, sizeof(buffer));
        return false;
      }
      ...
      if (fh->magic == FAT_CIGAM) {
        NSLog(@"[trolldecrypt] Executable is a FAT image - searching for right "
              @"architecture\n");
        arch = (struct fat_arch *)&fh[1];
        for (i = 0; i < swap32(fh->nfat_arch); i++) {
          if ((image_mh->cputype == swap32(arch->cputype)) &&
              (image_mh->cpusubtype == swap32(arch->cpusubtype))) {
            fileoffs = swap32(arch->offset);
            NSLog(
                @"[trolldecrypt] Correct arch is at offset 0x%x in the file.\n",
                fileoffs);
            break;
          }
          arch++;
        ...
        NSLog(@"[trolldecrypt] Executable is a plain MACH-O image, fileoffs = "
              @"0\n");
      } else {
      ...
      // NSLog(@"[trolldecrypt] restsize = %u, n = %u, cryptsize = %u, total =
      // %u", restsize, n, eic->cryptsize, n + eic->cryptsize + restsize);
      lseek(fd, 0, SEEK_SET);

      NSLog(@"[trolldecrypt] Copying the not encrypted start of the file (%u "
            @"bytes)\n",
            n);
      ...
      NSLog(@"[trolldecrypt] Dumping the decrypted data into the file (%u "
            @"bytes)\n",
            eic->cryptsize);
      buf = (char *)malloc((size_t)eic->cryptsize);
      readmem((mach_vm_offset_t *)buf, imageAddress + eic->cryptoff,
              eic->cryptsize, targetTask, &region_info);
      // r = write(outfd, (unsigned char *)image_mh + eic->cryptoff,
      // eic->cryptsize);
      ...
      NSLog(@"[trolldecrypt] Copying the not encrypted remainder of the file "
            @"(%u bytes)\n",
            restsize);
      ...
      if (off_cryptid) {
        uint32_t zero = 0;
        NSLog(@"[trolldecrypt] Setting the LC_ENCRYPTION_INFO->cryptid to 0 at "
              @"offset 0x%x into file\n",
              off_cryptid);
    ...
    // readmem((mach_vm_offset_t*)lc, lc+lc->cmdsize, header.sizeofcmds,
    // targetTask, &region_info);
  }
...
- (void)dumpDecrypted:(pid_t)pid {
  ...
  if (task_for_pid(mach_task_self(), pid, &targetTask)) {
    NSLog(@"[trolldecrypt] Can't execute task_for_pid! Do you have the right "
  ...
  if (task_info(targetTask, TASK_DYLD_INFO, (task_info_t)&dyld_info, &count) !=
      KERN_SUCCESS)
    exit(1);

  mach_msg_type_number_t size = sizeof(struct dyld_all_image_infos);
  uint8_t *data =
      readProcessMemory(targetTask, dyld_info.all_image_info_addr, &size);
  ...
  uint8_t *info_addr = readProcessMemory(
      targetTask, (mach_vm_address_t)infos->infoArray, &size2);
  ...
  for (int i = 0; i < numberOfImages; i++) {
    ...
    const char *imageName = (char *)readProcessMemory(
        targetTask, (mach_vm_address_t)info[i].imageFilePath, &size3);
    imageAddress = (vm_address_t)info[i].imageLoadAddress;
    ...
    if (strstr(imageName, appPath) != NULL) {
      ...
      [self dumpDecryptedImage:imageAddress
                      fileName:imageName
                         image:i
                          task:targetTask];
    }
...
- (BOOL)fileManager:(NSFileManager *)f
    shouldProceedAfterError:(BOOL)proceed
...
- (NSString *)IPAPath {
  return [NSString stringWithFormat:@"%@/%@_%@_decrypted.ipa", [self docPath],
...
- (void)createIPAFile:(pid_t)pid {
  ...
  NSString *appCopyDir =
      [NSString stringWithFormat:@"%@/ipa/Payload/%s", [self docPath],
                                 self -> appDirName];
  ...
  [fm createDirectoryAtPath:appCopyDir
      withIntermediateDirectories:true
                       attributes:nil
                            error:nil];
  ...
  NSLog(@"[trolldecrypt] ======== START FILE COPY - IGNORE ANY SANDBOX "
        @"WARNINGS ========");
                                 ...
                                 keepParentDirectory:NO
                                    compressionLevel:1
                                            password:nil
                                                 AES:NO
                                     progressHandler:nil];
    NSLog(@"[trolldecrypt] ========  ZIP operation complete: %s ========",
