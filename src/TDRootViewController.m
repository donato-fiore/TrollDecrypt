...
- (void)loadView {
  ...
  self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]
      initWithImage:[UIImage systemImageNamed:@"folder"]
              style:UIBarButtonItemStylePlain
             target:self
...
- (void)viewDidAppear:(bool)animated {
  ...
  fetchLatestTrollDecryptVersion(^(NSString *latestVersion) {
    ...
    NSLog(@"[trolldecrypter] Current version: %@, Latest version: %@",
          currentVersion, latestVersion);
    if (result == NSOrderedAscending) {
      dispatch_async(dispatch_get_main_queue(), ^{
        UIAlertController *alert = [UIAlertController
            alertControllerWithTitle:@"Update Available"
                             message:@"An update for TrollDecrypt is available."
...
- (void)openDocs:(id)sender {
  TDFileManagerViewController *fmVC =
      [[TDFileManagerViewController alloc] init];
  UINavigationController *navController =
      [[UINavigationController alloc] initWithRootViewController:fmVC];
  [self presentViewController:navController animated:YES completion:nil];
}

- (void)about:(id)sender {
  UIAlertController *alert = [UIAlertController
      alertControllerWithTitle:@"TrollDecrypt"
                       message:@"by fiore\nIcon by @super.user\nbfdecrypt by "
                               @"@bishopfox\ndumpdecrypted by @i0n1c\nUpdated "
                               @"for TrollStore by @wh1te4ever"
                preferredStyle:UIAlertControllerStyleAlert];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (([self.apps count] - 1) != indexPath.row) {
    ...
    cell.detailTextLabel.text = [NSString
        stringWithFormat:@"%@ • %@", app[@"version"], app[@"bundleID"]];
    cell.imageView.image = [UIImage
        _applicationIconImageForBundleIdentifier:app[@"bundleID"]
                                          format:iconFormat()
